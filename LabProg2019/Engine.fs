...
let mutable public Hide_FPS = false //this is tho hide the timer and the frame count, i show them only in the Pacman mode
...
type private 'st loop_data = { frame_cnt : int; elapsed : TimeSpan; now0 : DateTime; state : 'st; quit : bool }


type info = { frame_cnt : int; timer : double }
...
type engine (w : int, h : int, ?fps_cap : int, ?flip_queue) =
    ...
    member private this.shoot update info (data : _ loop_data) =
        ...
        let (st', quit'), ts = stopwatch_quiet <| fun () ->
            buff.lock_and_commit <| fun wr ->
                ...
                let st', quit' = update wr info data.state
                if this.show_sprites then lock sprites <| fun () -> for spr in sprites do spr.draw wr

                if this.show_fps  then //DA IMPLEMENTARE SOLO QUANDO C'è PACMAN  
                    let dt = DateTime.Now - data.now0
                    ...
                    wr.draw_text (hd, 3, h-8, Color.White, Color.Black)
                st', quit'
        { data with state = st'; frame_cnt = data.frame_cnt + 1; quit = quit'; elapsed = ts }
    
    //this is an identical copy of this shoot without  the print of frame count and frame time
    member private this.shoot2 (data : _ loop_data) st quit = 
        ...
        let (st', quit'), ts = stopwatch_quiet <| fun () ->
            buff.lock_and_commit <| fun wr ->
                ...
                let st' = st
                let quit' = quit
                if this.show_sprites then lock sprites <| fun () -> for spr in sprites do spr.draw wr//till here
                st', quit'
        { data with state = st'; frame_cnt = data.frame_cnt + 1; quit = quit'; elapsed = ts }
    //just for the autoresolution
    member this.refresh st quit=
        let mutable data = { state = st; frame_cnt = 0; quit = false; elapsed = new TimeSpan (); now0 = DateTime.Now }
    ...
    member this.loop_on_key update st0 =
        ...
        let mutable data = { state = st0; frame_cnt = 0; quit = false; elapsed = new TimeSpan (); now0 = now0 }
        data <- this.shoot (fun _ _ st -> st, false) { frame_cnt = data.frame_cnt; timer = 0. } data
        while not data.quit do
            let k = Console.ReadKey true
            engine.log_key_pressed k
            data <- this.shoot (update k) { frame_cnt = data.frame_cnt; timer = (DateTime.Now - now0).TotalSeconds } data
    ...
    member this.loop update st0 =
        ...
        let now0 = DateTime.Now
        let data = new synced<_> { state = st0; frame_cnt = 0; quit = false; elapsed = new TimeSpan (); now0 = now0 }
        ...
        let h _ (args : Timers.ElapsedEventArgs) =
            data.apply_and_set <| fun r ->
                ...
                let r = this.shoot (update ko) { frame_cnt = r.frame_cnt; timer = (DateTime.Now - now0).TotalSeconds } r
